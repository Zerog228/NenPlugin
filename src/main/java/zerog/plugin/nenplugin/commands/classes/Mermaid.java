...
public class Mermaid implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(command.getName().equalsIgnoreCase("set_mermaid") && sender instanceof Player p) {
            if(args.length > 0){
    ...
    public static void setMermaid(Player p){

        SetVillagerData.setAttributes(p);
        Monke.resetClassesPermissions(p);

        NenPlugin.addPermission(p.getUniqueId(), "nen.mermaid");
        ...
        PersistentData.setPlayerData(p, "data4", PersistentDataType.DOUBLE, 2.0, "");
        SetAttributes.SetAttributes(p, (byte) 4, 2.0);
        PersistentData.setPlayerData(p, "data5", PersistentDataType.DOUBLE, 4.1, "");
        SetAttributes.SetAttributes(p, (byte) 5, 4.1);
        SetAttributes.SetAttributes(p, (byte) 6, 0.0);
        PersistentData.setPlayerData(p, "class_number", PersistentDataType.INTEGER, 12, "");
    }
}
