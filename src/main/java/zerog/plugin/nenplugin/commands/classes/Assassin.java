...
public class Assassin implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(command.getName().equalsIgnoreCase("set_assassin") && sender instanceof Player p) {
            if(args.length > 0){
    ...
    public static void setAssassin(Player p){

        if(!p.hasPermission("nen.assassin")){
            Monke.resetClassesPermissions(p);
            NenPlugin.addPermission(p.getUniqueId(), "nen.assassin");
        }
        ...
        PersistentData.setPlayerData(p, "data1", PersistentDataType.DOUBLE, 0.108, "");
        SetAttributes.SetAttributes(p, (byte) 1, 0.108);
        ...
        PersistentData.setPlayerData(p, "data5", PersistentDataType.DOUBLE, 3.9, "");
        SetAttributes.SetAttributes(p, (byte) 5, 3.9);

        SetAttributes.SetAttributes(p, (byte) 6, 0.0);

        PersistentData.setPlayerData(p, "class_number", PersistentDataType.INTEGER, 16, "");
    }
}
