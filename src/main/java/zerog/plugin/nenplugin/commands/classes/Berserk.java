...
public class Berserk implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(command.getName().equalsIgnoreCase("set_berserk") && sender instanceof Player p) {
            if(args.length > 0){
    ...
    public static void setBerserk(Player p){

        SetVillagerData.setAttributes(p);
        Monke.resetClassesPermissions(p);

        NenPlugin.addPermission(p.getUniqueId(), "nen.berserk");
        ...
        PersistentData.setPlayerData(p, "data3", PersistentDataType.DOUBLE, 0.5, "");
        SetAttributes.SetAttributes(p, (byte) 3, 0.5);
        ...
        PersistentData.setPlayerData(p, "additional_upgrades", PersistentDataType.INTEGER, 2, "");
    }
}
