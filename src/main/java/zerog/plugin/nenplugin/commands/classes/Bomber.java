...
public class Bomber implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(command.getName().equalsIgnoreCase("set_bomber") && sender instanceof Player p){
            if(args.length > 0){
    ...
    public static void setBomber(Player p){

        if(!p.hasPermission("nen.bomber")){
            Monke.resetClassesPermissions(p);
            SetVillagerData.setAttributes(p);
        }


        PersistentData.setPlayerData(p, "class_number", PersistentDataType.INTEGER, 1, "");
        NenPlugin.addPermission(p.getUniqueId(), "nen.bomber");
