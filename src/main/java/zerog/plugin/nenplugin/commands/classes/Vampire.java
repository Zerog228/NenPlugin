...
public class Vampire implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(command.getName().equalsIgnoreCase("set_vampire") && sender instanceof Player p) {
            if(args.length > 0){
    ...
    public static void setVampire(Player p){

        if(!p.hasPermission("nen.vampire")){
            Monke.resetClassesPermissions(p);
            NenPlugin.addPermission(p.getUniqueId(), "nen.vampire");
        ...
        PersistentData.setPlayerData(p, "class_number", PersistentDataType.INTEGER, 9, "");
