...
public class Politic implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (command.getName().equalsIgnoreCase("set_politic") && sender instanceof Player p) {
            if(args.length > 0){
    ...
    public static void setPolitic(Player p){

        SetVillagerData.setAttributes(p);
        Monke.resetClassesPermissions(p);

        PersistentData.setPlayerData(p, "class_number", PersistentDataType.INTEGER, 6, "");
        NenPlugin.addPermission(p.getUniqueId(), "nen.politic");
        ...
        NenPlugin.addPermission(uuid, "infinitejump.use");
        NenPlugin.addPermission(uuid, "infinitejump.3");
    }
}
